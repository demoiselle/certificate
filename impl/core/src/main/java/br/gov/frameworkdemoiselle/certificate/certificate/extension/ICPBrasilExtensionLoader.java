...
import br.gov.frameworkdemoiselle.certificate.certificate.IOIDExtensionLoader;
		...
		if (field.isAnnotationPresent(ICPBrasilExtension.class)) {
			...
			try {
				...
				case DATA_NASCIMENTO:
					if (cert.hasCertificatePF()) {
						keyValue = cert.getICPBRCertificatePF().getDataNascimento().toString();
					} else {
						if (cert.hasCertificatePJ()) {
							keyValue = cert.getICPBRCertificatePJ().getDataNascimento().toString();
						} else {
							if (cert.hasCertificateEquipment()) {
